    ...
    if args.deepspeed:
        ...
        if model_engine.global_rank == 0:
            ...
            try:
                ...
                print(f"  ZeRO Stage: 3 (from config)")
            try:
                # Try to get offload info from config
                import json
                with open(args.deepspeed_config, 'r') as f:
                    ds_config = json.load(f)
                zero_config = ds_config.get('zero_optimization', {})
                param_offload = zero_config.get('offload_param', {}).get('device', 'none')
                opt_offload = zero_config.get('offload_optimizer', {}).get('device', 'none')
                print(f"  Parameter offloading: {param_offload}")
                print(f"  Optimizer offloading: {opt_offload}")
            except:
                print(f"  Offloading: Enabled (from config)")
