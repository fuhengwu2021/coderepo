""" 
Inference script using PagedAttention v2 with Online Softmax.

This script demonstrates how to use the PagedAttention v2 implementation
with online softmax (block-streaming) for KV cache management and attention
computation during decode for Qwen/Qwen2.5-0.5B-Instruct.
...
- Decode: run a minimal manual per-layer forward for ONE token, calling
  PagedAttentionV2.compute_attention(...) which uses online softmax to process
  blocks without concatenation.
"""
...
class PagedAttentionModelWrapperV2:
    """
    Wrapper around a HuggingFace model to use PagedAttention v2 (with online softmax).
    
    This version uses block-streaming attention computation that processes blocks
    one by one without concatenation, using online softmax algorithm.
    """
    ...
    def prefill(self, prompt: str, seq_id: Optional[int] = None) -> int:
        """
        Process the prompt and cache KV using PagedAttention v2.
    ...
    def decode_step(self, seq_id: int) -> Optional[int]:
        """Generate one token, using PagedAttention v2 (online softmax) for attention computation."""
    ...
    def generate(self, prompt: str, max_new_tokens: int = 50) -> str:
        """
        Generate text using PagedAttention v2.
