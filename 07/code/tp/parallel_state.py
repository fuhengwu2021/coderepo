"""
Tensor Parallelism State Management for SGLang-style TP.

This is adapted from chapter6's TP implementation but follows SGLang's approach
where TP is integrated with RadixAttention and scheduler.
"""
import torch
...
class TensorParallelGroup:
    ...
    def initialize(self, ranks: list[int], backend: str = "nccl"):
        ...
        # CPU group for Python object communication (like SGLang)
        self.cpu_group = dist.new_group(ranks, backend="gloo" if backend == "nccl" else backend)
        self.ranks = ranks
        self.world_size = len(ranks)
        self.first_rank = ranks[0]
