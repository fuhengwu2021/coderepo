...
kubectl create configmap llmd-api-gateway-code \
...
if [[ "$1" == "--restart" ]]; then
    ...
    kubectl rollout restart deployment/llmd-api-gateway -n "${NAMESPACE}"
    echo "✅ Gateway restart initiated"
    echo "   Check status: kubectl get pods -l app=llmd-gateway -n ${NAMESPACE}"
else
    echo ""
    echo "💡 To restart the Gateway:"
    echo "   kubectl rollout restart deployment/llmd-api-gateway -n ${NAMESPACE}"
    echo ""
